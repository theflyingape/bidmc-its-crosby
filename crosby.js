...
try {
    exports.appClientId = JSON.parse(fs.readFileSync(TOKEN_DIR + 'client_secret.json').toString());
}
catch (err) {
...
dns.lookup('0.0.0.0', (err, addr, family) => {
    ...
    app.get('/crosby/devices/', (req, res) => {
        authorize(exports.appClientId, (auth) => {
            let params = { auth: auth, customerId: 'my_customer', fields: 'chromeosdevices(annotatedAssetId,annotatedLocation,annotatedUser,deviceId,ethernetMacAddress,firmwareVersion,lastEnrollmentTime,lastSync,macAddress,meid,model,notes,orgUnitPath,osVersion,platformVersion,serialNumber,status,supportEndDate,bootMode)'
            };
            //	https://support.google.com/chrome/a/answer/1698333
            if (req.query.id)
                params.query = `id:${req.query.id}`;
            if (req.query.asset_id)
                params.query = `asset_id:${req.query.asset_id}`;
            if (req.query.ethernet_mac)
...
//# sourceMappingURL=crosby.js.map